...
namespace TMS.Services.Services
{
    public class DbMessageService : IDbMessageService
    {
        ...
        public DbMessageService(IBaseRepository<ProviderStatusCode, int> providerStatusCodes,
        ...
        public StatusCodeDTO GetMainStatusCodeMessage(int? id = null, int? providerId = null, string statusCode = "", string language = "en")
        {
            ...
            if (!providerId.HasValue)
            {
                response = _statusCodes.Getwhere(s => s.ID == id).Select(s => new StatusCodeDTO
                {
                    Id = s.ID,
                    Code = s.Code,
                    Message = language == "en" ? s.Message : s.ArMessage,
