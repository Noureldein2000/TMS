...
namespace TMS.Services.ProviderLayer
{
    public class BeniSuefUniversity : IBaseProvider
    {
        ...
        private readonly ITaxService _taxeService;
        ...
        public FeesResponseDTO Fees(FeesRequestDTO feesModel, int userId, int id)
        {
            ...
            var taxesList = _taxeService.GetTaxes(id, feesModel.Amount, feesModel.AccountId, feesModel.AccountProfileId, out decimal taxesAmount).ToList();
            ...
            else
            {
                if (taxesAmount.ToString() != "0.000")
                {
                    ...
                    _inquiryBillService.AddReceiptBodyParam(
                       new ReceiptBodyParamDTO
                       {
                           ParameterName = "Tax",// "Service Fees",
                           ProviderServiceRequestID = feesModel.Brn,
                           TransactionID = null,
                           Value = taxesAmount.ToString("0.000")
                       });
