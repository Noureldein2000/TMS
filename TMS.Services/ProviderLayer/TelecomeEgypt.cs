...
{
    public class TelecomeEgypt : IBaseProvider
    {
        ...
        public async Task<PaymentResponseDTO> Pay(PaymentRequestDTO payModel, int userId, int id, decimal totalAmount, decimal fees, int serviceProviderId)
        {
            ...
            var switchBodyRequest = new PaymentTelecomeEgypt
            {
                ...
                Fees = _providerService.GetProviderServiceResponseParams(payModel.Brn, "ar", "amountFees").Select(x => x.Value).FirstOrDefault()
            };
